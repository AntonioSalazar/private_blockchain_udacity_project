...
class Blockchain {
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            try {
                self.chain.map(star => {
                    let decodedData = JSON.parse(new Buffer.from(star.body, 'hex').toString())
                    decodedData.owner === address ? stars.push(decodedData) : null;
                })
                resolve(stars)
            } catch (error) {
                reject(`There has been an error: ${error}`)
            }
        });
    }
